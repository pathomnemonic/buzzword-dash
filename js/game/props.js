...
export function buildAmbulance() {
  ...
  const body = new THREE.Mesh(
    new THREE.BoxGeometry(2.5, 1.2, 1.4),
  ...
  const cab = new THREE.Mesh(
    new THREE.BoxGeometry(1.0, 0.9, 1.3),
    new THREE.MeshStandardMaterial({ color: 0xdddddd })
  );
  cab.position.set(-1.5, 0.85, 0);
  g.add(cab);

  const stripe = new THREE.Mesh(
    new THREE.BoxGeometry(2.5, 0.2, 1.42),
  ...
  crossH.position.set(0, 1.5, 0.72);
  g.add(crossH);
  ...
  crossV.position.set(0, 1.5, 0.72);
  g.add(crossV);
...
export function buildHospital() {
  ...
  for (let row = 0; row < 3; row++) {
    for (let col = 0; col < 4; col++) {
      ...
      win.position.set(-0.9 + col * 0.6, 1.2 + row * 1.1, 1.28);
      g.add(win);
    }
  }
...
export function buildHeartMonitor() {
  ...
  const base = new THREE.Mesh(
    new THREE.CylinderGeometry(0.4, 0.5, 0.1, 12),
...
export function buildSyringe() {
  ...
  const barrel = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xddddff, transparent: true, opacity: 0.7 })
  );
  ...
  const needle = new THREE.Mesh(
    new THREE.CylinderGeometry(0.01, 0.005, 0.4, 6),
...
export function buildMedicalCross() {
  ...
  const h = new THREE.Mesh(new THREE.BoxGeometry(0.4, 1.2, 0.1), mat);
  h.position.set(0, 2.5, 0);
  g.add(h);

  const v = new THREE.Mesh(new THREE.BoxGeometry(1.2, 0.4, 0.1), mat);
  v.position.set(0, 2.5, 0);
  g.add(v);
