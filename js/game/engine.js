 ...
 * - FIX #11: Reset camera and player in doContinue()
 * - FIX #12: Reset camera lookAt in endRun()
 * - FIX #13: Reset camera lookAt in start()
 * - FIX #14: Track name moved to go() so it shows after countdown
 * - FIX #15: cleanupTrack() uses protectedSet for PowerUpFX and TrailSystem
 * - FIX #16: Rush propulsion calculates exact speed for 0.5s gate arrival
 * - FIX #17: Autopilot auto-rushes when collected during active gates
 */
...
class Game {
  constructor() {
    ...
    // FIX #16: Rush speed override for precise gate arrival
    ...
    // FIX #7: Monster starts at Z=20 (behind camera, positive Z)
    this.monsterZ = 20;
    this.monsterTargetZ = 20;
    this.monsterVisible = false;
    this.monsterWarningPlayed = false;

    // FIX #5: Heart spawn threshold restored to 3
  ...
  updateExamMonster(dt) {
    ...
    // FIX #7: Monster visible when Z < 16 (camera is at Z=10, looking at Z=-20)
  ...
  addRushStack() {
    ...
    if (this.rushStacks < this.maxRushStacks) {
      ...
      // FIX #16: Calculate speed needed to reach gate in 0.5 seconds
  ...
  // FIX #2 + #15: cleanupTrack protects PowerUpFX groups and TrailSystem particles
  cleanupTrack() {
    var toRemove = [];
    var self = this;
  ...
  // FIX #11: Reset camera and player state in doContinue()
  doContinue() {
  ...
  start(mode) {
    ...
    // FIX #16: Reset rush speed override
    this.rushSpeedOverride = 0;
    ...
    // FIX #14: Track name moved to go() - don't show here
  ...
  go() {
    ...
    audio.startAmbient(this.currentSkin.name);
    this.spawnEncounter();
    // FIX #14: Show skin name after countdown in go()
    if (this.onSkinSelected) this.onSkinSelected(this.currentSkin.name);
  }
  ...
  // FIX #10: Monster consume with dramatic camera
  ...
  update(dt) {
    ...
    // FIX #6 + #16: During rush propulsion, use calculated speed to guarantee gate arrival
  ...
  endRun() {
    ...
    // FIX #12: Reset faceplant state and camera lookAt in endRun()
    this.faceplanting = false;
    this.camera.position.copy(this.cameraBasePos);
    this.camera.fov = this.baseFOV;
    this.camera.updateProjectionMatrix();
