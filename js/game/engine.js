...
class Game {
  ...
  update(dt) {
    ...
    if (this.trackRefs) {
      ...
      // FIX: Wall scroll panels now updated
      ...
      if (c.position.z > -3 && c.position.z < 2 && !c.userData.collected) {
        ...
        if (inLane || closeEnough || magnetActive) {
          ...
          if (c.userData.type === 'powerup') {
            ...
            // FIX: Pass lane for stereo panning
            audio.play('coin', c.userData.lane);
            // Quest: q_50coins (target is 100)
  ...
  endRun() {
    ...
    if (this.mode === 'daily') {
      ...
      if (lastDaily) {
        ...
        // If lastDaily === today, don't change streak (already played today)
      } else {
        storage.set('dailyStreak', 1);
      }
