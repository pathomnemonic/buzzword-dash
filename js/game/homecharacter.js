 ...
 * - 8-12 flying medical props from props.js constantly drifting past
 ...
 *
 * The Three.js canvas can render behind HTML with transparent
 * backgrounds using alpha:true on the renderer and transparent
 * CSS backgrounds on the overlaid elements [5].
 */
...
// Gesture types for idle animation
var GESTURE_NONE = 0;
...
export class HomeCharacter {
    ...
    init(existingRenderer) {
        ...
        // Fog for depth
        this.scene.fog = new THREE.FogExp2(0x120828, 0.025);
    ...
    _spawnOneProp(randomizeZ) {
        ...
        // Scale down so they feel like background elements
        var scale = 0.15 + Math.random() * 0.25;
        prop.scale.set(scale, scale, scale);

        // Make semi-transparent for depth feel
        prop.traverse(function (child) {
            if (child.material) {
                if (Array.isArray(child.material)) {
                    child.material.forEach(function (m) {
                        m.transparent = true;
                        m.opacity = Math.min(m.opacity || 1, 0.5 + Math.random() * 0.3);
                    });
                } else {
                    child.material.transparent = true;
                    child.material.opacity = Math.min(child.material.opacity || 1, 0.5 + Math.random() * 0.3);
                }
            }
        });
