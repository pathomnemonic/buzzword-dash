...
export class PowerUpFX {
  ...
  // ===== COIN MAGNET =====
  // Golden glow aura + orbiting ring + arc lines to coins
  ...
  update(dt, playerPos, powerups, rushBoostStacks) {
    ...
    for (var key in this.effects) {
      if (this.effects[key].group) {
        this.effects[key].group.position.set(playerPos.x, playerPos.y, playerPos.z);
      }
    }
    ...
    for (var i = 0; i < 12; i++) {
      ...
      (function(s, sc) {
        ...
        function animateShard() {
          ...
          s.position.x += s.userData.vel.x * 0.016;
          s.position.y += s.userData.vel.y * 0.016;
          s.position.z += s.userData.vel.z * 0.016;
          s.userData.vel.y -= 9.8 * 0.016;
