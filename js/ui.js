...
class UI {
  ...
  // FIX: Live millisecond speed timer that accepts the game object
  ...
  renderSettings() {
    ...
    // FIX: Reset uses reload instead of re-running init()
  ...
  updateHud(game) {
    ...
    // FIX: Pass game object for live speed timer
  ...
  showPostRun(game) {
    ...
    // FIX: Trigger confetti on personal best
    if (game.isNewBest) {
      this.showConfetti();
    }
    ...
    // FIX: Calendar uses local date key
  ...
  // FIX: Calendar uses local date keys and hasOwnProperty for 0% accuracy
  renderCalendar() {
