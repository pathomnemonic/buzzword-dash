...
*
* NEW additions:
* - Vignette overlay during rush (Item 11)
* - Coin trail animation to HUD (Item 13)
* - Speed timer toggle via triple-tap on speed dial (Item 16)
* - Home character start/stop integration (Item 3)
* - Golden Doctor hidden shop section (Item 15)
* - Confetti burst on personal best
* - Quick Review mode for missed cards
...
// ===== NEW: Home character reference (set by main.js) =====
this.homeCharacter = null;  

// ===== NEW: Speed timer triple-tap tracking =====
this.speedDialTapCount = 0;
this.speedDialTapTimer = null;  

// ===== NEW: Vignette element reference =====
...
// ===== NEW: Create vignette overlay element =====
...
// ===== NEW: Update vignette intensity based on rush stacks =====
updateRushVignette(rushStacks) {
if (!this.vignetteEl) return;
if (rushStacks > 0) {
this.vignetteEl.classList.add('active');
// Scale intensity with rush stacks (1-3)
var intensity = Math.min(rushStacks, 3);
var spread = 120 - intensity * 20; // 100, 80, 60
var alpha = 0.3 + intensity * 0.15; // 0.45, 0.60, 0.75
...
// ===== NEW: Coin trail animation to HUD =====
showCoinTrail() {
...
// Get target position (coin counter in HUD)
var rect = coinCounter.getBoundingClientRect();
var targetX = rect.left + rect.width / 2;
var targetY = rect.top + rect.height / 2;  

// Animate after a frame to trigger CSS transition
...
// ===== NEW: Speed timer display in HUD =====
updateSpeedTimer(lastEncounterTime) {
if (!storage.get('speedTimerEnabled')) return;
var timerEl = document.getElementById('hudSpeedTimer');
if (!timerEl) return;
if (lastEncounterTime > 0) {
timerEl.textContent = '\u23F1 ' + (lastEncounterTime / 1000).toFixed(2) + 's';
...
this.renderHome();
// ===== NEW: Start home character animation =====
...
// ===== NEW: Stop home character when leaving home =====
...
// ===== NEW: Stop home character when hiding all screens =====
...
// ===== NEW: Triple-tap speed dial to toggle speed timer (Item 16) =====
...
// ===== NEW: Filter out hidden Golden Doctor until unlocked (Item 15) =====
if (type === 'skin') {
...
// ===== NEW: Update rush vignette =====
this.updateRushVignette(game.rushStacks);  

// ===== NEW: Update speed timer =====
if (game.lastEncounterTime > 0) {
this.updateSpeedTimer(game.lastEncounterTime);
}
}  
...
// ===== NEW: Golden Doctor unlock notification in post-run =====
var goldenNotice = '';
...
// ===== Hide vignette after run ends =====
this.updateRushVignette(0);
}  

// ===== NEW: CONFETTI BURST ON PERSONAL BEST =====
